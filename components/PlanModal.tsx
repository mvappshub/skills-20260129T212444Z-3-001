      ...
      items: type === EventType.PLANTING ? [
        {
          ...
          species_name_latin: species || 'Neurèeno',
      ...
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6 overflow-y-auto">
          ...
          <div className="space-y-4">
            <div>
              ...
              <input
                ...
                placeholder={type === EventType.PLANTING ? "Napø. Alej svobody" : "Napø. Zálivka stromù"}
            ...
            <div>
              ...
              <input
                type="text"
                value={isGeocoding ? 'Naèítám adresu...' : (address || '')}
              ...
              />
              <p className="text-xs text-slate-500 mt-1">Adresa se doplòuje automaticky podle zvoleného místa.</p>
            </div>
          </div>
          ...
          <div className="bg-slate-50 p-4 rounded-lg border border-slate-200">
            ...
            <div className="flex flex-col gap-3">
              ...
              <div className="flex gap-2 flex-wrap">
                ...
                >
                  <MapPin size={16} />
                  {pickedLocation ? 'Zmìnit na mapì' : 'Vybrat na mapì'}
                </button>
                <button
                  type="button"
                  onClick={() => {
                ...
                >
                  ...
                  {geoLoading ? 'Naèítám...' : 'Moje poloha'}
                </button>
              </div>
              {geoError && (
            ...
            <div className="space-y-4 border-t border-slate-100 pt-4">
              ...
              <div className="grid grid-cols-3 gap-4">
                <div className="col-span-2">
                  <label className="block text-xs font-medium text-slate-500 mb-1">Døevina (Latinský název)</label>
                  <input
                    type="text"
                    value={species}
                    onChange={(e) => setSpecies(e.target.value)}
                    placeholder="Napø. Tilia cordata"
                    className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-emerald-500 outline-none"
                  />
                </div>
                <div>
                  <label className="block text-xs font-medium text-slate-500 mb-1">Poèet (ks)</label>
                  <input
                    type="number"
                    min="1"
                    value={quantity}
          ...
          {type === EventType.MAINTENANCE && (
            <div className="space-y-4 border-t border-slate-100 pt-4">
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">Poznámka k údržbì</label>
                <textarea
                  value={notes}
                  onChange={(e) => setNotes(e.target.value)}
                  rows={3}
                  placeholder="Popis èinnosti..."
          ...
          {!isLocationSelected && (
            <p className="text-xs text-amber-600">Vyberte místo na mapì nebo použijte GPS.</p>
          )}
          {isGeocoding && (
            <p className="text-xs text-slate-500">Naèítám adresu, prosím poèkejte...</p>
          )}
        </form>
